...
| FUNC-037 | Alertas: pestaña Total + auto-refresh 60s + cache de rendimiento; Dashboard: fix timeline de colaboradores | Completada | 2026-08-06 |
| FUNC-038 | Dashboard: score de eficiencia bayesiano (Colaborador/Líder) mensual; Alertas: guardado de estado instantáneo | Completada | 2026-08-12 |
...
### [FUNC-037] Alertas: pestaña Total + auto-refresh 60s + cache de rendimiento; Dashboard: fix timeline de colaboradores
- **Estado:** Completada
- **Fecha:** 2026-08-06
- **Descripcion:** (1) Nueva pestaña **"Total"** en el Listado de Alertas que muestra todas las alertas de todos los estados (abiertas y cerradas), sin el filtro de Pendientes/Finalizadas. (2) **Auto-refresh cada 60s**: los datos de Alertas se refrescan solos (silencioso, sin recargar la página) cada 60 segundos, al volver a la pestaña del navegador (`visibilitychange`) y tras guardar un cambio de estado — resuelve el caso de un usuario viendo una alerta como "pendiente" cuando otro ya la resolvió. Se pausa mientras hay una fila en edición para no perder cambios a medio hacer. (3) **Optimización de rendimiento** que hizo viable el punto (2) sin degradar el sistema: la carga de Alertas bajó de ~12-13s a ~6s en frío / ~1s con cache, mediante `$first=10000` en la consulta a DAB (antes 100, paginaba secuencialmente ~70 veces) + cache en memoria de 15s compartido entre todos los usuarios conectados (con invalidación inmediata al cambiar el estado de una alerta, para que quien hizo el cambio lo vea reflejado al instante). (4) **Dashboard de Tareas** — "Comparativa de tareas completadas" (timeline por colaborador): antes solo listaba colaboradores que ya tenían alguna tarea "Completa" en el rango; ahora incluye a todos los que tienen alguna tarea (completada o no) en el rango, para que el selector pueda ofrecer a cualquiera y el usuario decida agregarlos/quitarlos (decisión tomada con el usuario vía pregunta explícita).
- **Backend:** `DabAlertasClient.cs` — cache en memoria (`IMemoryCache`, TTL 15s, key `dab:notificaciones:todas`) para `GetAllAsync`, invalidado en `ActualizarAsync`; paginación con `$first=10000` en vez de 100; timeout del `HttpClient` de DAB subido de 30s a 60s. `DependencyInjection.cs` — `services.AddMemoryCache()`. `GetDashboardQueryHandler.cs` — `CalculateCompletionTimeline` agrupa por todos los colaboradores con tareas en el rango (antes filtraba `Status == Completa` ANTES de agrupar, excluyendo del todo a quienes no tuvieran ninguna completada).
- **Frontend:** `AlertasPayroll.jsx` — `refreshData()` (fetch silencioso, no bloquea la tabla) separado de `loadData()` (carga inicial con spinner); `useEffect` con `setInterval(60000)` + listener `visibilitychange`, pausado cuando `editingRow` está activo; botón "Actualizar" y `handleSave` usan `refreshData()`; nueva sub-pestaña "Total" (`tableTab === 'todas'`, sin filtro de estado) con contador `stats.total`.
- **Validado en producción (solo lectura, vía SSH/Mongo):** el comportamiento de "colaboradores sin tareas no aparecen en el gráfico" en el Dashboard de Gerente se confirmó igual que en local (19 colaboradores activos, 16 con al menos 1 tarea histórica, 3 sin ninguna) — no es un bug, es reflejo real de los datos; se deja tal cual a pedido del usuario.
- **Pendiente (sin cambios de código, solo diagnóstico):** rol "Soporte Alertas" reportado como que seguía filtrando alertas por usuario — causa real: el rol se había creado como "Soporte" en vez de "Soporte Alertas" (no coincidía con el nombre exacto que espera `AlertaRolesPrivilegiados`), corregido por el usuario del lado de datos. De paso quedó identificado un bug real en `LoginCommandHandler.cs`: el arreglo `rolePriority` (`Administrador > Gerente > Lider > Colaborador`) no incluye "Soporte Alertas", por lo que si algún usuario llega a combinar ese rol con cualquiera de esos 4, el otro rol le gana como rol activo y pierde el acceso sin restricción — no corregido aún, pendiente de decisión.
- **Security:** Sin cambios de superficie de ataque (cache en memoria de solo lectura server-side, sin exponer datos nuevos; el resto es UI/rendimiento).

---

### [FUNC-038] Dashboard: score de eficiencia bayesiano (Colaborador/Líder) mensual; Alertas: guardado de estado instantáneo
- **Estado:** Completada
- **Fecha:** 2026-08-12
- **Descripcion:** (1) **"Colaborador más eficiente" / "Líder más eficiente"** (Dashboard de Tareas) dejan de calcularse por suma de tiempo estimado y pasan a usar un **promedio bayesiano** sobre el Rating de tareas completadas: `score = (promedio_persona × n + promedio_global × K) / (n + K)`, con `K=10` (validado contra volumen real de producción: mediana ~40 tareas/mes por colaborador, ~31 por líder). Evita que 1-2 tareas con calificación perfecta le ganen a alguien con un promedio sostenido sobre muchas tareas — decisión tomada junto al usuario tras comparar esta opción contra un filtro de "mínimo de tareas para calificar" (descartado por dejar la tarjeta vacía cuando nadie llega al mínimo). **Alcance temporal:** por defecto es del **mes actual** (mismo criterio que "Estrella del mes": `DueDate`, con `CreatedAt` de respaldo); si el usuario filtra el Dashboard por un rango de fechas, se usa ese rango en su lugar. (2) **Alertas — guardar cambio de estado**: bajó de ~7s a ~1-2s. Antes, al guardar se esperaba un refetch completo de las ~10000 filas (invalidado el cache de 15s por el propio cambio) antes de cerrar el modal; ahora el backend devuelve los valores que quedaron aplicados y el frontend actualiza esa fila al instante en la tabla local, sin esperar red — el resto de la tabla se sigue sincronizando solo con el auto-refresh de 60s. Además, overlay de "Guardando cambios…" sobre el modal mientras dura la operación (antes solo cambiaba el texto del botón, poco visible), y el timer del mensaje "Guardado" se acortó de 1.8s a 900ms.
- **Backend:** `DashboardDto.cs` — `HighlightLabels` con `FastestCollaboratorAvgRating/Score` y `TopLeaderAvgRating/Score`. `GetDashboardQueryHandler.cs` — `CalculateHighlights` reescrito (score bayesiano sobre `Rating` de tareas `Completa`, agrupando por `AssignedToName`/`AssignedLeaderName`, `EfficiencyScoreK=10`); `Handle` calcula `highlightTasks` acotado al mes actual salvo que `request.From/To` traigan un rango explícito. `ChangeAlertaStatusDto.cs`/`ChangeAlertaStatusHandler.cs` — `ChangeAlertaStatusResultDto` ahora incluye `Estado`, `NotasResolucion`, `UsuarioResolucion` (los mismos valores ya aplicados en el PATCH a DAB, sin round-trip adicional).
- **Frontend:** `Home.jsx` — `HighlightCards` muestra `{avgRating}% promedio ({count} tareas) · score {score}` en vez de horas estimadas; card de líder renombrada a "Líder más eficiente". `AlertasPayroll.jsx` — `refreshData()` devuelve la promesa; `handleSave` hace merge local de `estado/notasResolucion/usuarioResolucion` en `data` (en vez de `refreshData()` bloqueante) y dispara un `refreshData()` en segundo plano tras cerrar; overlay `payroll-saving-overlay` (spinner + "Guardando cambios…") mientras `saving` está activo. `AlertasPayroll.css` — estilos del overlay (`position:relative` en `.preview-modal`, overlay `position:absolute` con `z-index` sobre el contenido).
- **Validado:** build backend (0 errores) y frontend limpios; sin tests existentes sobre Dashboard que cubrieran el cálculo anterior. Caso límite revisado con datos locales: un líder con 1 sola tarea calificada al 100% puede seguir ganando el ranking pese al ajuste bayesiano si su promedio real supera al promedio global del equipo — matemáticamente correcto (el score no garantiza "más volumen gana", solo evita que muestras chicas se sobre-premien cuando su calidad real no lo amerita); documentado y explicado al usuario, sin cambios adicionales pedidos.
- **Pendiente de despliegue:** cambios probados y desplegados solo en **local** (docker local + `html/`); no se han llevado a producción todavía.
- **Security:** Sin cambios de superficie de ataque (cálculo server-side sobre datos ya expuestos; el guardado de alertas sigue exigiendo el módulo `alertas-payroll` y JWT válido).
