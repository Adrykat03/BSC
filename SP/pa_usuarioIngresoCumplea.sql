			...
			IF (@tiene1 <> 0)
				...
				if @html1 is not null 
					...
					begin
							...
							VALUES ('A', 'CambioCumple', 'pa_usuarioIngresoCumplea', @asunto, @html, @tiene1, @destinatarios, NULL, NULL, 'Con novedad', 'Alta', 'PRT - HORARIOS Y MARCACIONES', NULL);
	...
	END
		ELSE
			BEGIN
						...
						-- INSERT notificación consolidada
						INSERT INTO Avisos.notificacionesConsolidadas (estado, origen, spOrigen, asunto, descripcionHtml, cantidadRegistros, destinatarios, periodoInicio, periodoFin, descripcion, prioridad, categoria, mensajeError)
						VALUES ('C', 'CambioCumple', 'pa_usuarioIngresoCumplea', @asunto, @html1, @tiene1, @destinatarios, NULL, NULL, 'Sin novedad', 'Alta', 'PRT - HORARIOS Y MARCACIONES', NULL);
						exec msdb.dbo.Sp_send_dbmail
							@profile_name = 'Informacion_Nomina', 
							@Subject = @asunto,
							@recipients = @destinatarios,
