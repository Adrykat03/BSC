USE [ADAM]
GO
/****** Object:  StoredProcedure [dbo].[sp_fpv_aviso_vacaciones_descuadre_mes]        Script Date: 5/6/2026 16:50:38 ******/
SET ANSI_NULLS ON
GO
...
-----Fecha 13/07/2016
----Version 1.2
----Recorre la tabla saldo vacaciones para buscar a quien se le diferencia el mes de entrada en la antiguedad, con la fecha de comienzo del ciclo.
...
-- =============================================
ALTER PROCEDURE [dbo].[sp_fpv_aviso_vacaciones_descuadre_mes]
AS
...
BEGIN
        ...
        WHERE T.sit_trabajador = 1
            ...
            AND datepart(month, T.fecha_antiguedad) <> datepart(month, V.fecha_ini_prog_vac)
        ...
        BEGIN
			   ...
			   , @Subject = @asunto
                ...
                EXEC sp_fpv_inserta_avisos_vacaciones 'PTO_A2_Mes'
                        , @mensaje
                        , @ciclo
                        , @dia
                        , @mes
        ...
        BEGIN
                EXEC msdb.dbo.Sp_send_dbmail        @profile_name = 'Informacion_Nomina',
			           @Subject = ' Punto A.2 Aviso Descuadre de mes en vacaciones'
                        , @recipients = @correo
                        , @body_format = 'html'
                        , @copy_recipients = @correoCC
        ...
        set @html_nc =
            ...
            '<tr><td style="padding:4px;">&nbsp;Punto A.2: Error de descuadre en el mes de inicio de vacaciones</td>' +
            '<td style="text-align:center;padding:4px;">' + CAST(@totalRegistros as varchar(10)) + '</td>' +
            '<td style="text-align:center;padding:4px;"><a href="https://nomina.kfc.com.ec/KFCReporteador/vacaciones/AvisosVacaciones.aspx?A1=37" target="_blank">Ver Informe</a></td>' +
            '</tr></table></div>'
        ...
        VALUES
            (@estado, 'Mail_AvVac', 'sp_fpv_aviso_vacaciones_descuadre_mes',
             'Punto A.2: Error de descuadre en el mes de inicio de vacaciones',
